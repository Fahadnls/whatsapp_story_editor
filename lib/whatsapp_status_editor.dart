library whatsapp_status_editor;
...
import 'story_editor/components/background_gradient_selector_widget.dart';
import 'story_editor/components/footer_tools_widget.dart';
import 'story_editor/components/overlay_item_widget.dart';
import 'story_editor/components/remove_widget.dart';
import 'story_editor/components/size_slider_widget.dart';
import 'story_editor/components/text_color_select_widget.dart';
import 'story_editor/components/text_field_widget.dart';
import 'story_editor/components/top_tools_widget.dart';
import 'story_editor/constants/font_colors.dart';
import 'story_editor/constants/gradients.dart';
import 'story_editor/constants/item_type.dart';
...
// The stack data for the editable items.
  ...
  void _init() async {
    _stackData.add(EditableItem()..type = ItemType.IMAGE);
    ...
    _onScreenTap();
    setState(() {
      _selectedTextBackgroundGradient = 12;
      _selectedTextColorIndex = 0;
      _selectedTextColor = const Color(0xffffffff);
    });
  }
        ...
        body: Stack(
          ...
          children: [
            ...
            TopToolsWidget(
              ...
              onToggleBackgroundColorPicker: _onTabBackgroundGradientTap,
  ...
  void _onToggleTextColorSelector() {
    ...
    if (_selectedTextColorIndex < defaultColors.length - 1) {
      setState(() {
        _selectedTextColorIndex++;
        _selectedTextColor = defaultColors[_selectedTextColorIndex];
      });
    } else {
      setState(() {
        _selectedTextColorIndex = 0;
  ...
  void _onTabBackgroundGradientTap() {
    HapticFeedback.lightImpact();
    if (_selectedBackgroundGradient < gradientColors.length - 1) {
      setState(() {
        _selectedBackgroundGradient++;
      });
    } else {
  ...
  void _onScaleUpdate(ScaleUpdateDetails details) {
    ...
    final left =
        (delta.dx / MediaQuery.of(context).size.width) + _currentPos.dx;
    final top =
        (delta.dy / MediaQuery.of(context).size.height) + _currentPos.dy;
  ...
  Future<void> _onDone() async {
    ...
    await await Future.delayed(const Duration(
      milliseconds: 800,
    ));
    ...
    final imageData = await imgFile.writeAsBytes(pngBytes);
    _isLoading = false;
    setState(() {});
    delayFunction(
        milliseconds: 500, todo: () => Navigator.pop(context, imageData));
  }
